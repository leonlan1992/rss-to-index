...
```
*/15 * * * * cd /绝对路径/rss-to-index && /绝对路径/python3 run.py
```

（加 `--quiet` 可以只写日志文件、不打屏。）
...
**为什么用关键词挂股票，而不是让模型判断。** 让 LLM 自由发挥打公司标签，前后会不一致（今天叫 META、明天叫 Meta Platforms、后天叫 Facebook），所以无论哪种方式都需要 `config/stocks.csv` 这张表当标准答案集。往表上匹配有三档：关键词（本项目用的，便宜、可复现，但会误判——"高盛发报告称 A 公司产品暴雷"会把高盛也挂上）；把股票池表连同新闻丢给 LLM 以 JSON 返回（千级 token，能分清语义）；向量化（池子特别大时）。从第一档起步，被误判烦到了再升级。
...
- **URL 去重挡不住转载。** 同一件事被十家媒体报道就是十个 URL。Techmeme 本身做过人工聚合，单源问题不大；多源之后可以通过向量化做进一步去重。
