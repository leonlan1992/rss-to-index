...
"""入口。
    ...
    python run.py                # 完整跑一遍
    python run.py --dry-run      # 只抓取 + 本地打标，不调 Gemini、不写 Notion
...
def main() -> int:
    parser = argparse.ArgumentParser(description="RSS → 翻译 → 打标 → Notion")
    parser.add_argument("--limit", type=int, default=None, help="本次最多处理多少条")
    parser.add_argument("--dry-run", action="store_true",
                        help="不调 Gemini、不写 Notion，只抓取和本地打标")
    ...
    return 1 if pipeline.had_failures(stats) else 0
